...
public class ArrayTest {
	...
	public static void main(String[] args) {
		...
		// index : 0 ~ 요소의 개수-1
		// 요소의 개수는 배열이름.length 인스턴스 변수에 저장
		...
		System.out.println("arr 배열의 요소들의 합 :" + sum);
		...
		int[] ref;
		ref = addAllArray(arr4, 7); // addAllArray(0x100, 7)
...
//		}
		...
		int j = 0;		// 임의의 index
		int temp = 0;	// 각 변수의 값을 바꾸기위한 임시 변수
		
		for(int i = 0; i<100; i++) {
			// 임의의 index를 구한다 : 1 ~ 44
			j = (int)(Math.random()*43)+1;
			temp = ball[0];
			ball[0] = ball[j];
			ball[j] = temp;
			}
